# ==========================================
# ui_formulas.py
# Responsible for formula calculation and process monitoring (Think Tank Analysis)
# ==========================================
    ...
    with st.expander(t("🧮 Game Formulas & Calculation Monitor (Think Tank Analysis)"), expanded=False):
        ...
        if game.phase == 1:
            if getattr(game, 'p1_selected_plan', None):
                ...
                # Fetch draft for the currently operating role (R or H)
        ...
        # Define status tags
        TAG_EST = " (Est)"  # Estimate (from Think Tank)
        TAG_TMP = " (Tmp)"  # Temporary (Draft phase)
        TAG_CFM = " (Cfm)"  # Confirmed (Passed third reading or existing asset)
        TAG_CON = " (Const)"  # Constant (System fixed parameter)
        ...
        # Dynamic bid data determination
        ...
        # 3. Fund Check and GDP Settlement
        ...
        # Check if calculating is possible
        ...
        if can_calc:
            ...
            if avail_v >= req_v:
                ...
                st.error(f"❌ Funding Gap (Available {avail_v:.1f} < Required {req_v:.1f}), plan proportionally unfinished.")
            ...
            st.info("💡 Some key data (like rewards or benefits) is still `pending (Empty)`, cannot complete prediction.")
