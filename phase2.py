...
def render(game, view_party, opponent_party, cfg):
    ...
    tot_maint = inv_pre + inv_inv + inv_med + inv_stl + inv_bld
    tot = tot_action + tot_maint
    
    st.write(f"**總花費:** `{tot:.0f}` / **剩餘可用淨值:** `{cw - tot:.0f}`")
    
    if tot > cw:
        st.error(f"🚨 資金不足！當前預算已超支，請降低投入資金。")
        ...
        acts_dict = {
            ...
            'policy': tot_action, 'maint': tot_maint
        }
        st.session_state[f"{view_party.name}_acts"] = acts_dict
        view_party.last_acts = acts_dict.copy()
