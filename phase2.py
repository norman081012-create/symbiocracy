...
def render(game, view_party, opponent_party, cfg):
    ...
    with c1:
        ...
        else: st.caption("💡 **監管系統特性**: 根據情報處準確度抓捕貪污圖利")
    ...
    ui_core.render_dashboard(game, view_party, opponent_party, cfg, is_preview=True, preview_data=preview_data)
    ...
    if tot <= current_wealth and st.button("確認行動/結算", use_container_width=True, type="primary"):
        ...
        if f"{opponent_party.name}_acts" not in st.session_state:
            ...
            if ha['corr'] > 0:
                # 抓貪準確度 = 監管情報處 - 執行反情報處
                eff_inv = max(0.0, rp.investigate_ability - hp.covert_ability)
            ...
            if game.year % cfg['ELECTION_CYCLE'] == 1:
                ...
                if game.ruling_party.name != winner.name:
                    st.session_state.news_flash = f"🎉 **【大選結果】** {winner.name} 以 {winner.support:.1f}% 的支持率勝出，成為當權！"
                else: st.session_state.news_flash = f"🎉 **【大選結果】** {winner.name} 成功連任當權！"
