...
with st.sidebar:
    ...
    st.session_state.god_mode = st.toggle("👁️ 上帝視角", False)
...
st.markdown(f"## 🏛️ Symbiocracy 共生民主模擬器 v3.0.0")
st.subheader(f"📅 遊戲年份: {cfg['CALENDAR_NAME']} {game.year} 年 ({elec_status})")

if st.session_state.get('god_mode'): st.error(f"👁️ **上帝視角：** 真實衰退率為 **{game.current_real_decay:.2f}**")
...
ui_core.render_party_cards(game, view_party, st.session_state.get('god_mode'), is_election_year, cfg)
