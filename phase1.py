...
def render(game, view_party, cfg):
    ...
    if game.p1_step in ['draft_r', 'draft_h', 'ultimatum_draft_r']:
        ...
        if game.p1_step == 'ultimatum_draft_r': st.error("🚨 **最後通牒啟動中：** 監管系統必須擬定最終裁決草案！")
        ...
        if view_party.name != (game.r_role_party.name if active_role == 'R' else game.h_role_party.name):
            ...
            with c_decay:
                opp_txt = f"對手公告: {opp_plan['claimed_decay']:.2f}" if opp_plan else "等待對手公告"
            ...
            r_pct = (r_pays / max(1, req_funds)) * 100; h_pct = (h_pays / max(1, req_funds)) * 100
            
            st.markdown(f"### 監管出資 {r_pays} ({r_pct:.1f}%) / 總額: {req_funds} / 執行出資 {h_pays} ({h_pct:.1f}%)")
            ...
            if active_role == 'R' and game.p1_step == 'draft_r':
                if c_btn2.button("💥 發布最後通牒", use_container_width=True):
                    game.p1_selected_plan = plan_dict; game.p1_step = 'ultimatum_resolve_h'; game.proposing_party = game.h_role_party
                    st.session_state.news_flash = f"📢 **【快訊】監管系統下達最後通牒！** {view_party.name} 逼迫執行系統必須接受此案！"
                    st.rerun()
            ...
            with c_left:
                my_net, my_roi = (o_h_n, o_h_roi) if my_is_h else (o_r_n, o_r_roi)
                st.markdown(f"🛡️ **依據自己智庫估算** *(衰退估算: -{view_party.current_forecast:.2f})*")
                st.markdown(f"🟢 我方預估收益: {my_net:.0f} (ROI: {my_roi:.1f}%)")
                st.markdown("---")
                my_net_c, my_roi_c = (c_h_n, c_h_roi) if my_is_h else (c_r_n, c_r_roi)
                st.markdown(f"📢 **依據方案公告估算** *(衰退估算: -{claimed_decay:.2f})*")
                st.markdown(f"🟢 我方預估收益: {my_net_c:.0f} (ROI: {my_roi_c:.1f}%)")

            with c_right:
                if opp_plan:
                    st.markdown("📜 **對手 (執行系統) 既有草案參考**")
                    st.write(f"公告衰退: {opp_plan['claimed_decay']:.2f} | 目標 GDP 成長: {opp_plan['target_gdp_growth']}%")
                    st.write(f"總額: {opp_plan['total_funds']} (監管 {opp_plan['r_pays']} / 執行 {opp_plan['h_pays']})")
                else: st.markdown("📜 **對手 (執行系統) 既有草案參考**\n*(尚未發布)*")
                st.markdown("---")
                st.markdown("📊 👁️ **本黨智庫推演報告:**")
                st.info(f"預期 GDP: `{game.gdp:.0f} ➔ {o_est_gdp:.0f}` ({o_gdp_pct:+.2f}%)")

    elif game.p1_step == 'voting_pick':
        st.markdown(f"### 🗳️ 當權派定奪 ({game.ruling_party.name})")
        if view_party.name != game.ruling_party.name: st.warning("⏳ 等待定奪...")
        else:
            cols = st.columns(2)
        ...
        if view_party.name != game.proposing_party.name: st.warning("⏳ 等待對手覆議...")
            ...
            if c1.button("✅ 同意法案", use_container_width=True, type="primary"):
                st.session_state.turn_data.update(game.p1_selected_plan); game.phase = 2; game.proposing_party = game.ruling_party; st.rerun()
            ...
            if c3.button(f"🔄 同意但換位\n(扣款 {penalty_amt})", use_container_width=True):
                st.session_state.turn_data.update(game.p1_selected_plan); engine.trigger_swap(game, penalty_amt, "執政權轉移！"); game.proposing_party = game.ruling_party; st.rerun()
            if game.proposing_party.name == game.h_role_party.name:
                if c4.button("💥 逼迫最終提案", use_container_width=True):
                    game.p1_step = 'ultimatum_draft_r'; game.proposing_party = game.r_role_party; st.rerun()

    elif game.p1_step == 'ultimatum_resolve_h':
        if view_party.name != game.h_role_party.name: st.warning("⏳ 等待決斷...")
        else:
            ui_core.render_proposal_component('📜 監管系統最終底線/通牒方案', game.p1_selected_plan, game, view_party, cfg)
            c1, c2 = st.columns(2)
            if c1.button("✅ 忍辱負重", use_container_width=True, type="primary"):
                st.session_state.turn_data.update(game.p1_selected_plan); game.phase = 2; game.proposing_party = game.ruling_party; st.rerun()
            if c2.button(f"🔄 掀桌倒閣換位", use_container_width=True):
                st.session_state.turn_data.update(game.p1_selected_plan); engine.trigger_swap(game, penalty_amt, "掀桌倒閣！"); game.proposing_party = game.r_role_party; st.rerun()
