...
def render(game, view_party, cfg):
    ...
    st.subheader(t(f"🤝 Phase 1: R-System Commissions H-System Construction Proposal (Round: {game.proposal_count})"))
    ...
    if game.p1_step in ['draft_r', 'draft_h', 'ultimatum_draft_r']:
        ...
        if view_party.name != (game.r_role_party.name if active_role == 'R' else game.h_role_party.name):
            ...
            with c_ann1:
                ...
                st.markdown(t(f"**Claimed Decay (Current: {current_val:.3f}) (Equals {req_infra_to_balance:.1f} infra)** | {opp_txt1}"))
            ...
            proj_fund = st.slider(t("Plan Reward (Reward for H 100% completion, max = total budget)"), 0.0, max_budget, def_proj, 10.0)
            bid_cost = st.slider(t("Total Plan Benefit (Infra produced at 100% completion)"), 0.0, max_budget * 1.5, def_bid, 10.0)
            r_pays = st.slider(t("💰 R-Pays (Subsidy paid from R's budget)"), 0.0, max_budget, def_rpays, 10.0)
            ...
            if c_btn1.button(t("📤 Submit Standard Draft"), use_container_width=True, type="primary", disabled=is_invalid):
                if game.p1_step == 'ultimatum_draft_r':
            ...
            if active_role == 'R' and game.p1_step == 'draft_r':
                if c_btn2.button(t("💥 Issue Ultimatum"), use_container_width=True, disabled=is_invalid):
                    ...
                    st.session_state.news_flash = t(f"🗞️ **[BREAKING] R-System Ultimatum!** {view_party.name} forces H-System to accept or trigger a swap!")
                    st.rerun()
                
                swap_cost = 0 if view_party.name == game.ruling_party.name else penalty_amt
                if c_btn3.button(t(f"🔄 Force Pass & Swap\n(Cost: Both pay {swap_cost:.1f} to charity)"), use_container_width=True, disabled=is_invalid):
        ...
        if view_party.name != game.ruling_party.name:
            st.warning(t("⏳ Waiting for ruling party decision..."))
        else:
