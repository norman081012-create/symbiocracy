...
def render_formula_panel(game, view_party, cfg):
    # 📌 [DEBUG WRAP] 把整個函式包在 try/except 裡。
    # Streamlit Cloud 對「未捕捉」的例外會 redact 訊息內容，
    # 但如果我們自己 catch 住再用 st.exception() 印出來，就會是完整未過濾的 traceback。
    # 等你重現一次錯誤、把畫面上這段完整 traceback 貼給我之後，
    # 就可以把這層 try/except 拿掉，換成真正對症的修法。
    try:
        _render_formula_panel_inner(game, view_party, cfg)
    except Exception as e:
        st.error("⚠️ Formula panel 發生錯誤（已攔截，不影響其他功能）。請把下面完整內容貼給 Claude：")
        st.exception(e)

def _render_formula_panel_inner(game, view_party, cfg):
